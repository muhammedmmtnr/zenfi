import 'package:flutter/material.dart';
import 'package:zenfi/model/employe.dart';
import 'package:zenfi/model/list_data_model.dart';

class EmployeeListController with ChangeNotifier {
  final List<ListDataModel> list = [
    ListDataModel(
      title: 'Kieth Wyatt',
      subTitle: '480-220-3011',
      trailing:
          'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=687&q=80',
    ),
    ListDataModel(
      title: 'Wade Vincent',
      subTitle: '601-337-2386',
      trailing:
          'https://images.unsplash.com/photo-1499996860823-5214fcc65f8f?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=766&q=80',
    ),
    ListDataModel(
      title: 'Doug Gibson',
      subTitle: '504-238-8498',
      trailing:
          'https://images.unsplash.com/photo-1502823403499-6ccfcf4fb453?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=687&q=80',
    ),
    ListDataModel(
      title: 'Nate Robinson',
      subTitle: '305-334-8193',
      trailing:
          'https://images.unsplash.com/photo-1522075469751-3a6694fb2f61?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=880&q=80',
    ),
    ListDataModel(
      title: 'Mark Thomas',
      subTitle: '631-608-1670',
      trailing:
          'https://images.unsplash.com/photo-1531727991582-cfd25ce79613?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=687&q=80',
    ),
  ];

  void addEmployee(Employee employee) {
    list.add(ListDataModel(
      title: employee.title,
      subTitle: employee.subTitle,
      trailing:
          'assets/images/image-placeholder.png', // You can set a default image or handle this dynamically
    ));
    notifyListeners(); // Notify listeners of change
  }
}
